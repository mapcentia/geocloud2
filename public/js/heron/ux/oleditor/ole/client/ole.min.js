...
fillOpacity:.8,strokeColor:"#070",strokeWidth:2,graphicZIndex:1,pointRadius:5}),select:new OpenLayers.Style({fillColor:"#fc0",strokeColor:"#f70",graphicZIndex:2}),temporary:new OpenLayers.Style({fillColor:"#fc0",fillOpacity:.8,strokeColor:"#f70",strokeWidth:2,graphicZIndex:2,pointRadius:5})}),this.map.addLayer(this.sourceLayers[d]);this.map.editor=this;this.map.addLayer(this.editLayer);this.map.addControl(new OpenLayers.Editor.Control.LayerSettings(this));this.undoRedoActive&&this.map.addControl(new OpenLayers.Editor.Control.UndoRedo(this.editLayer));
...
CLASS_NAME:"OpenLayers.Editor.Control.DeleteFeature"});OpenLayers.Editor.Control.Dialog=OpenLayers.Class(OpenLayers.Control,{dialogDiv:null,buttonClass:null,inputTextClass:null,modal:!0,initialize:function(a){OpenLayers.Control.prototype.initialize.apply(this,[a])},show:function(a){var b,c;-1<OpenLayers.Util.indexOf(this.map.viewPortDiv.childNodes,this.dialogDiv)&&this.map.viewPortDiv.removeChild(this.dialogDiv);a||(a={});this.dialogDiv=document.createElement("div");OpenLayers.Element.addClass(this.dialogDiv,"oleDialog");a.toolbox?OpenLayers.Element.addClass(this.dialogDiv,
...
this.layer.events.triggerEvent("afterfeaturemodified",{feature:f});break a}}}},CLASS_NAME:"OpenLayers.Editor.Control.DrawHole"});OpenLayers.Editor.Control.DrawPolygon=OpenLayers.Class(OpenLayers.Control.DrawFeature,{minArea:0,title:OpenLayers.i18n("oleDrawPolygon"),initialize:function(a,b){this.callbacks=OpenLayers.Util.extend(this.callbacks,{point:function(a){this.layer.events.triggerEvent("pointadded",{point:a})}});OpenLayers.Control.DrawFeature.prototype.initialize.apply(this,[a,OpenLayers.Handler.Polygon,b]);this.title=OpenLayers.i18n("oleDrawPolygon")},drawFeature:function(a){var b=new OpenLayers.Feature.Vector(a);!1!==
...
var c=document.getElementById("olLabelInput"),d=this;c.onkeypress=function(a){13==(window.event?window.event.keyCode:a.keyCode)&&(d.setLabelText(b,c.value),d.removePopup(b))};c.focus();this.popup=a},CLASS_NAME:"OpenLayers.Editor.Control.DrawText"});OpenLayers.Editor.Control.UploadFeature=OpenLayers.Class(OpenLayers.Control,{EVENT_TYPES:["featureuploaded"],layer:null,title:OpenLayers.i18n("oleUploadFeature"),url:"",params:{action:"upload",mime:"text/html",encoding:"escape"},formats:[{name:"GeoJSON",fileExt:".json",mimeType:"text/plain",formatter:"OpenLayers.Format.GeoJSON"}],fileProjection:null,visibleOnUpload:!0,formatters:{},replaceFeatures:!1,initialize:function(a,b){this.layer=a;this.title=OpenLayers.i18n("oleUploadFeature");OpenLayers.Control.Button.prototype.initialize.apply(this,
...
"oleUploadFeatureReplace";d.appendChild(document.createTextNode(OpenLayers.i18n("oleUploadFeatureReplace")));c.appendChild(d);b.appendChild(c);for(var k in this.params)b=this.createInputElm(null,k,"hidden",this.params[k],b);a.appendChild(b);this.showUploadDialog(a);var m=this;document.getElementById("oleFile").onchange=function(a){if(a=m.fileInputValue(this))if(a=a.split("."),1<a.length){a="."+a[a.length-1].toLowerCase();for(var b=document.getElementById("format_select"),c=b.options,d=0;d<c.length;++d)if(a==
...
b,c);if(!a._moving){var f=a.dragControl.handlers.drag.evt,g=!(!a._setfeature&&a.preserveAspectRatio)&&!(f&&f.shiftKey),h=new OpenLayers.Geometry.Point(d,e),f=a.center;this.rotate(-a.rotation,f);h.rotate(-a.rotation,f);var k=this.x-f.x,l=this.y-f.y,m=k-(this.x-h.x),n=l-(this.y-h.y);a.irregular&&!a._setfeature&&(k-=(this.x-h.x)/2,l-=(this.y-h.y)/2);this.x=d;this.y=e;h=1;a.feature.geometry instanceof OpenLayers.Geometry.Point?g=1:g?(g=1E-5>Math.abs(n)?1:l/n,h=(1E-5>Math.abs(m)?1:k/m)/g):g=Math.sqrt(k*
k+l*l)/Math.sqrt(m*m+n*n);a._moving=!0;a.box.geometry.rotate(-a.rotation,f);delete a._moving;a.box.geometry.resize(g,f,h);a.box.geometry.rotate(a.rotation,f);a.transformFeature({scale:g,ratio:h});a.irregular&&!a._setfeature&&(k=f.clone(),k.x+=1E-5>Math.abs(d-f.x)?0:this.x-d,k.y+=1E-5>Math.abs(e-f.y)?0:this.y-e,a.box.geometry.move(this.x-d,this.y-e),a.transformFeature({center:k}))}},f=function(b,c){var d=this.x,e=this.y;OpenLayers.Geometry.Point.prototype.move.call(this,b,c);if(!a._moving){var f=a.dragControl.handlers.drag.evt,
f=f&&f.shiftKey?45:1,g=a.center,h=this.x-g.x,k=this.y-g.y;this.x=d;this.y=e;d=Math.atan2(k,h)-Math.atan2(k-c,h-b);d*=180/Math.PI;a._angle=(a._angle+d)%360;d=a.rotation%f;if(Math.abs(a._angle)>=f||0!==d)d=Math.round(a._angle/f)*f-d,a._angle=0,a.box.geometry.rotate(d,g),a.transformFeature({rotation:d})}},g=Array(8),h=Array(4),k,m,n,p="sw s se e ne n nw w".split(" "),l=0;8>l;++l)k=this.box.geometry.components[l],m=new OpenLayers.Feature.Vector(k.clone(),{role:p[l]+"-resize"},"string"==typeof this.renderIntent?
null:this.renderIntent),0==l%2&&(n=new OpenLayers.Feature.Vector(k.clone(),{role:p[l]+"-rotate"},"string"==typeof this.rotationHandleSymbolizer?null:this.rotationHandleSymbolizer),n.geometry.move=f,k._rotationHandle=n,h[l/2]=n),k.move=b,k.resize=c,k.rotate=d,m.geometry.move=e,k._handle=m,g[l]=m;this.rotationHandles=h;this.handles=g}});OpenLayers.Editor.Control.FixedAngleDrawing=OpenLayers.Class(OpenLayers.Control,{CLASS_NAME:"OpenLayers.Editor.Control.FixedAngleDrawing",active:!1,sketchVerticesAmount:null,guides:null,initialize:function(a){this.guides=[];OpenLayers.Control.prototype.initialize.call(this);this.layer=a},activate:function(){var a=OpenLayers.Control.prototype.activate.call(this);if(a)this.layer.events.on({sketchstarted:this.onSketchStarted,sketchmodified:this.onSketchModified,sketchcomplete:this.onSketchComplete,scope:this});
...
OpenLayers.Handler.Path){c=d.handler.layer;break}}c.events.on({featureremoved:function(a){a.feature.id===b.id&&(this.sketchVerticesAmount=null,this.getSnappingGuideLayer().destroyFeatures())},scope:this})}},getSnappingGuideLayer:function(){return this.map.getLayersByClass("OpenLayers.Editor.Layer.Snapping")[0]},updateGuideLines:function(a,b){var c=this.getSnappingGuideLayer(),d=-1/((b.y-a.y)/(b.x-a.x));this.guides.push(c.addLine({m:d,b:b.y-d*b.x,x:b.x}))}});OpenLayers.Editor.Control.Gc2Load=OpenLayers.Class(OpenLayers.Control.Button,{layer:null,initialize:function(a,b){this.layer=a;OpenLayers.Control.Button.prototype.initialize.apply(this,[b]);this.trigger=this.load;this.title=OpenLayers.i18n("Load drawings from disk")},load:function(){var a=new OpenLayers.Format.WKT,b=this.layer,c=function(){$.get(host+"/controllers/drawing",function(c,e){b.addFeatures(a.read(c.data));MapCentia.gc2.map.zoomToExtent(b.getDataExtent(),!1)},"json")};if(0<b.features.length)if(confirm("Are you sure? Any unsaved drawings will be lost!"))b.destroyFeatures(),
c();else return!1;else c()},CLASS_NAME:"OpenLayers.Editor.Control.Gc2Load"});OpenLayers.Editor.Control.Gc2Save=OpenLayers.Class(OpenLayers.Control.Button,{layer:null,title:OpenLayers.i18n("oleImportFeature"),initialize:function(a,b){this.layer=a;OpenLayers.Control.Button.prototype.initialize.apply(this,[b]);this.trigger=this.save;this.title=OpenLayers.i18n("Save drawing to disk")},save:function(){var a=this.layer.features,a=(new OpenLayers.Format.WKT).write(a);$.post(host+"/controllers/drawing",a,function(a,c){},"text")},CLASS_NAME:"OpenLayers.Editor.Control.Gc2Save"});OpenLayers.Editor.Layer=OpenLayers.Class(OpenLayers.Layer,{initialize:function(a){OpenLayers.Control.prototype.initialize.apply(this,[a])},CLASS_NAME:"OpenLayers.Editor.Layer"});OpenLayers.Editor.Layer.Snapping=OpenLayers.Class(OpenLayers.Layer.Vector,{CLASS_NAME:"OpenLayers.Editor.Layer.Snapping",intersectionPoints:null,initialize:function(a,b){this.intersectionPoints=[];void 0===b.styleMap&&(b.styleMap=new OpenLayers.StyleMap({"default":new OpenLayers.Style({strokeColor:"#ff00ff",strokeOpacity:.5,strokeWidth:1,strokeDashstyle:"solid",fillColor:"#ff00ff"},{rules:[new OpenLayers.Rule({evaluate:function(a){return a.geometry instanceof OpenLayers.Geometry.LineString},symbolizer:{strokeDashstyle:"longdash"}}),
...
f.b,d),f=this.createWorldBoundaryPoint(f.m,d.right,f.b,d));d=a.geometry.components[0];d.x=c.x;d.y=c.y;var g=a.geometry.components[1];g.x=f.x;g.y=f.y;c=OpenLayers.Layer.Vector.prototype.drawFeature.apply(this,arguments);d.x=e.x1;d.y=e.y1;g.x=e.x2;g.y=e.y2}else console.warn("Failed to render a feature",a)}return c},moveTo:function(a,b,c){OpenLayers.Layer.Vector.prototype.moveTo.apply(this,arguments);this.features.forEach(function(a){this.drawFeature(a)},this)}});OpenLayers.Editor.Control.SnappingSettings=OpenLayers.Class(OpenLayers.Control.Button,{title:OpenLayers.i18n("oleSnappingSettings"),layer:null,snapping:new OpenLayers.Control.Snapping,tolerance:10,snappingLayers:null,snappingGuideLayer:null,layerListDiv:null,toleranceInput:null,initialize:function(a,b){this.snappingLayers=[];this.layer=a;OpenLayers.Control.Button.prototype.initialize.apply(this,[b]);this.trigger=OpenLayers.Function.bind(this.openSnappingDialog,this);this.events.register("deactivate",
...
this.currentState&&this.undoStack.push(this.currentState);this.currentState=c;this.redoStack=[]},CLASS_NAME:"OpenLayers.Editor.Control.UndoRedo"});OpenLayers.Editor.Control.CADTools=OpenLayers.Class(OpenLayers.Control.Button,{title:OpenLayers.i18n("oleCADTools"),layer:null,parallelDrawingButton:null,guidedDrawingButton:null,tolerance:10,fixedAngleDrawingControl:null,parallelDrawingControl:null,snappingControl:null,initialize:function(a,b){this.layer=a;this.fixedAngleDrawingControl=new OpenLayers.Editor.Control.FixedAngleDrawing(a);this.parallelDrawingControl=new OpenLayers.Editor.Control.ParallelDrawing(a);OpenLayers.Control.Button.prototype.initialize.apply(this,
