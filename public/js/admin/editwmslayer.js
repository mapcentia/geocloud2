...
wmsLayer.init = function (record) {
    ...
    wmsLayer.grid = new Ext.grid.PropertyGrid({
        ...
        propertyNames: {
            ...
            opacity: 'Opacity',
            label_max_scale: __('Label max scale denominator') + __('Maximum scale at which this LAYER is labeled. Scale is given as the denominator of the actual scale fraction, for example for a map at a scale of 1:24,000 use 24000.', true), //LABELMAXSCALEDENOM
            label_min_scale: __('Label min scale denominator') + __('Minimum scale at which this LAYER is labeled. Scale is given as the denominator of the actual scale fraction, for example for a map at a scale of 1:24,000 use 24000.', true), //LABELMINSCALEDENOM
            cluster: 'Clustering distance',
            maxscaledenom: __('Max scale denominator') + __('Maximum scale at which this LAYER is drawn. Scale is given as the denominator of the actual scale fraction, for example for a map at a scale of 1:24,000 use 24000.', true),
            minscaledenom: __('Min scale denominator') + __('Minimum scale at which this LAYER is drawn. Scale is given as the denominator of the actual scale fraction, for example for a map at a scale of 1:24,000 use 24000.', true),
    ...
    wmsLayer.qmlForm = new Ext.FormPanel({
        frame: false,
        border: false,
        id: "qmlForm",
        labelWidth: 1,
        ...
        items: [
            {
                html: '<table>' +
                    '<tr class="x-grid3-row"><td><b>QML</b></td></tr>' +
                    '</table>',
                border: false,
                bodyStyle: 'padding-left: 3px'
            },
            ...
            },
            {
                name: 'wmssource',
                xtype: 'hidden',
                value: record.wmssource

            },
            {
                name: 'wmsclientepsgs',
                xtype: 'hidden',
                value: record.wmsclientepsgs

            },
        ],
        buttons: [
            {
                text: '<i class="fa fa-check"></i> ' + __('Update QML'),
                handler: function () {
                    ...
                    if (f.form.isValid()) {
                        ...
                        Ext.Ajax.request({
                            ...
                            success: function () {
                                store.reload();
                                writeFiles(record._key_);
                                App.setAlert(App.STATUS_NOTICE, "QML updated");
                            },
                            failure: function (response) {
                                Ext.MessageBox.show({
                                    title: 'Failure',
