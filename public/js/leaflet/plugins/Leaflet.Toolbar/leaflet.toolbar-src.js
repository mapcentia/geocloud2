(function(window, document, undefined) {
	...
	L._Toolbar = (L.Layer || L.Class).extend({
		statics: {
	...
	L._Toolbar.extend = function extend(props) {
	...
	L._ToolbarAction = L.Handler.extend({
		statics: {
		...
		initialize: function(options) {
			var defaultIconOptions = L._ToolbarAction.prototype.options.toolbarIcon;
	...
	L._toolbarAction = function toolbarAction(options) {
		return new L._ToolbarAction(options);
	};

	L._ToolbarAction.extendOptions = function(options) {
		return this.extend({ options: options });
	};

	L._Toolbar.Control = L._Toolbar.extend({
		statics: {
			baseClass: 'leaflet-control-toolbar ' + L._Toolbar.baseClass
		},

		initialize: function(options) {
			L._Toolbar.prototype.initialize.call(this, options);

			this._control = new L.Control._Toolbar(this.options);
		},

		onAdd: function(map) {
			this._control.addTo(map);

			L._Toolbar.prototype.onAdd.call(this, map);

			this.appendToContainer(this._control.getContainer());
		},

		onRemove: function(map) {
			L._Toolbar.prototype.onRemove.call(this, map);
			if (this._control.remove) {this._control.remove();}  // Leaflet 1.0
			else {this._control.removeFrom(map);}
		}
	});

	L.Control._Toolbar = L.Control.extend({
		onAdd: function() {
			return L.DomUtil.create('div', '');
		}
	});

	L._toolbar.control = function(options) {
		return new L._Toolbar.Control(options);
	};

// A convenience class for built-in popup toolbars.

	L._Toolbar.Popup = L._Toolbar.extend({
		statics: {
			baseClass: 'leaflet-popup-toolbar ' + L._Toolbar.baseClass
		},
		...
		initialize: function(latlng, options) {
			...
			/*
			 * Developers can't pass a DivIcon in the options for L._Toolbar.Popup
	...
	L._toolbar.popup = function(options) {
		return new L._Toolbar.Popup(options);
	};
