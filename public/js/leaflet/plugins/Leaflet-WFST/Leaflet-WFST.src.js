...
L.XmlUtil = {
  ...
  parseXml: function (rawXml) {
	  ...
	  rawXml = rawXml.replace(/\r/g,' ');
	  rawXml = rawXml.replace(/>\s+</g, "><");
...
L.GML.PointNode = L.GML.Geometry.extend({
  ...
  parse: function (element) {
    return this.parseElement(get_firstchild(element), {dimensions: this.dimensions(element)});
  }
});
...
L.GML.FeatureType = L.Class.extend({
  ...
  parse: function (feature) {
	...
	if (typeof get_firstchild(feature).attributes['fid'] !== "undefined") {
		id = get_firstchild(feature).attributes['fid'].value; 	
	} else if(typeof get_firstchild(feature).attributes['gml:id'] !== "undefined"){
		id = get_firstchild(feature).attributes['gml:id'].value;	
	} else {
...
L.Format.GML = L.Format.Base.extend({
  ...
  generateLayer: function (feature) {
    ...
    return this.parseElement(get_firstchild(geometryField), this.options);
  }
});
