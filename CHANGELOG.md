...
- New v4 Map API `api/v4/map/schema/{schema}`, the token-authenticated port of the legacy GUI `/controllers/setting/extent` endpoint. `GET` returns the per-schema initial map view (`center`, `zoom`, `extent`); `PATCH` sets any subset of them (partial update leaves the others intact, `null` clears a value). Values are stored in the same `settings.viewer` document (`center`/`zoom`/`extents` keyed by schema) so existing readers (Mapfile, schema rename) keep working, and the new model methods take the schema explicitly instead of the process-global connection schema (worker/FrankenPHP-safe). The legacy endpoint is unchanged.
