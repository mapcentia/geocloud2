...
- New v4 endpoint to delete a MapCache tileset's cached tiles: `DELETE api/v4/mapcache/database/{database}/tileset/{tileset}` (optionally scoped by `?bbox=minx,miny,maxx,maxy` and `?zoom=minzoom,maxzoom`). Deletion is delegated to `mapcache_seed -m delete`, which handles every configured cache backend (disk, sqlite, bdb, s3, memcache) and supports scoped invalidation, so a full-tileset delete over a large cache never has to be re-implemented per backend or block the request. The seed process is launched detached in the background and tracked in `settings.seed_jobs` (the tile seeder's table); the call returns `202 Accepted` with a job uuid/pid that the existing tileseeder tooling can poll or kill. Deletion requires write/owner authorization (Bearer token or HTTP Basic; superuser, schema owner, or `read/write` on the layer) regardless of the layer's read-auth level. Replaces the partial legacy `/controllers/setting`-era deletion, which only covered sqlite/disk (bdb was broken, s3/memcache unsupported) and used a blocking, injection-prone `exec("rm -R …")`.
