...
## [Unreleased]
### Added
- **Parquet snapshots.** New v4 job API `POST /api/v4/snapshots` (`{schema, relation, srs?}`, one object or an array of them — with an array every relation is checked before anything is queued) answering `202`, with `GET /api/v4/snapshots[/{id}]` for status (`pending/running/succeeded/failed/superseded`; comma separated ids answer with an array). A cron worker (`app/scripts/snapshot_worker.php`, under `flock`) exports the relation with ogr2ogr to GeoParquet (Parquet for non-spatial relations) and publishes it through a storage abstraction (`app/inc/snapshot/`: `s3` via Flysystem/AWS SDK, or `local`), partitioned as `{prefix}/{database}/schema=X/relation=Y/_gc2_snapshot_date=YYYY-MM-DD/data-<uuid>.parquet` + `metadata-<uuid>.json` (both carry a `schema_version` hash and the column schema). Publishing is one transaction that supersedes the previous snapshot of the relation; rows left `running` for two hours are reclaimed. Configured by the new `snapshot` block in `App.php` (`storage`, `bucket`/`prefix` or `localPath`, `download` = `proxy` or `redirect` with presigned URLs, `urlTtl`); a missing configuration answers `501 SNAPSHOT_NOT_CONFIGURED`. Super-user only.
- **Snapshots in several formats (Parquet + FlatGeobuf, extensible).** `POST /api/v4/snapshots` takes an optional `formats` array (`parquet`, `flatgeobuf`); when omitted the server default `snapshot.formats` in `App.php` applies (default `["parquet"]`, also used by scheduler jobs with `snapshot: true`). The worker exports every requested format with ogr2ogr (layer named after the relation) into the same partition as `data-<uuid>.parquet` / `data-<uuid>.fgb`; a format that cannot be produced for a relation (FlatGeobuf needs a geometry or geography column) is skipped and recorded with a reason, and the snapshot fails only when nothing could be produced. Requesting only geometry formats for a non-spatial relation is refused with 400. The snapshot row and `metadata-<uuid>.json` list `formats` as `{format, status: requested|produced|skipped, file, size_bytes, media_type, reason}`. The read API gains `GET/HEAD …/snapshots/{date}/data/{format}` (HEAD, byte ranges, proxy or presigned redirect, the format's media type); `/data` keeps serving the Parquet, and the metadata carries a `href` per produced format. The STAC item gets one asset per produced format (`data` for Parquet, `flatgeobuf` for FlatGeobuf). All formats live in one registry class (`SnapshotFormat`), so a new format is one entry plus tests. Migration adds `settings.snapshots.formats`; existing snapshots are unaffected.
- Scheduler run log: everything `get.php` prints once its run is registered is also stored on the run row (`started_jobs.log`, updated at every heartbeat and on finish, capped at 1 MB with the tail kept) and returned as `log` by `GET /api/v4/scheduler/runs/{uuid}`; listings stay without it. stdout and the per-job log file are unchanged.
- **Per-job snapshot formats on scheduler jobs.** A scheduler job can name the formats of the snapshot it queues after a successful import: `snapshot_formats` on `POST`/`PATCH /api/v4/scheduler/jobs` (a non-empty, duplicate-free list of `parquet`/`flatgeobuf`; `null` — the default — keeps the server default `snapshot.formats` from `App.php`). Stored in the new `gc2scheduler.jobs.snapshot_formats` column and passed on to `get.php`; existing jobs are unaffected.
- **Static STAC catalog in the snapshot store.** Publishing a Parquet snapshot now also (re)writes a browsable STAC 1.1.0 catalog of the database next to the data: `{prefix}/{database}/catalog.json` (one child per relation), `schema=X/relation=Y/collection.json` (title, abstract and tags from the layer metadata, spatial extent as the union of the snapshots' footprints, temporal extent oldest to newest, `gc2:schema_version`/`proj:code` summaries) and `schema=X/relation=Y/_gc2_snapshot_date=D/item.json` per snapshot (bbox polygon, `gc2:row_count`, `gc2:snapshot_id`, the CRS as `proj:code` under the projection extension v2, and the Parquet and metadata files as assets). Every link is relative to the file that carries it, so the catalog works unchanged on S3 and on local disk and validates with pystac/STAC Browser. The relation's WGS84 bounding box is computed at export time and stored both in `metadata-<uuid>.json` and in the new `settings.snapshots.bbox` column; snapshots published before this change get an item without geometry. The rebuild happens after every successful publish (and after a supersede) and is best effort: if it fails the snapshot still counts as succeeded and the reason is logged.
- **Snapshot read API** `GET /api/v4/schemas/{schema}/relations/{relation}/snapshots[/{date}[/data|/files/{name}]]`: list published snapshots (bare array, newest first), one snapshot's metadata, and its files with `HEAD`, `Range`/`206`, `ETag`, `Content-Length` and CORS preflight, so DuckDB, pandas and browsers can `read_parquet` straight over HTTP. Sub-users may read what their layer privileges allow; geofence `deny`/`limit` rules block the download. `download: redirect` answers with a presigned S3 URL instead of proxying.
- **Scheduler v4 API.** `api/v4/scheduler/jobs/[id]` (CRUD; `schedule` is a cron expression; `GET`/`DELETE` take comma separated ids, `POST` one object or an array — a bad list creates or deletes nothing) and `api/v4/scheduler/runs/[uuid]` (list runs with `status`, `host`, `slot`, `heartbeat`, `stale`, `exit_reason`; `POST {job}` starts a job manually (`202`); `DELETE` stops a running run with SIGINT, escalating to SIGKILL after 30 s). The v3 scheduler API lists the same registry and gained `DELETE /api/v3/scheduler/{uuid}`.
- Scheduler cooldown: `gc2scheduler.minInterval` (seconds, default `0`) in `App.php` is the minimum time between two runs of the same job. A cron tick inside the window is recorded as `skipped` with the reason, instead of running — protection against jobs left on `* * * * *`. Manual starts (API, admin GUI) bypass the cooldown.
- `AGENTS.md` (imported by `CLAUDE.md`) with the repository's developer rules: v4 API design principles (routes, scopes, the one-of id/batch design, response shapes, error codes, OpenAPI), worker-safe controller rules, background job conventions, testing and process.

- feat(v4): OGC API Features (Part 1 Core, Part 2 CRS) and OGC API Maps (Part 1 Core) under `api/v4/ogc/database/{database}`: landing page, conformance, collections, streamed GeoJSON items with bbox/limit/offset/crs/datetime, and PNG/JPEG maps through the WMS backend. Geofence rules, versioning (time slice via `datetime`) and workflow are enforced by the WFS engine and OWS proxy. A map request whose WMS backend answers with something other than an image (MapServer error page, ServiceException) is answered as a `502` JSON error (`MAP_BACKEND_ERROR`) instead of streaming the error page through with `200`. Addressing a `Read/write` collection without sufficient access answers `401` (with a Basic challenge) for anonymous callers and `403` for identified ones; only an unknown collection is `404`, while the collections list simply omits what the caller may not read. `Ows.php`/`Wfs.php` now share `PublicIdentity`, `LayerGate` and `RuleFilters`; the `*` anonymous geofence identity applies to these v4 WFS/OGC endpoints, not the legacy `/wfs/` bootstrap. Upgrade note: `f=jpeg` maps require the WMS mapfiles to be regenerated once (saving any layer via `POST /api/v4/layers`, or the legacy `/controllers/mapfile` endpoint), because the `jpeg` OUTPUTFORMAT is emitted by `Mapfile::generateWms()`.
- New v4 Keyvalue API `api/v4/keyvalue/{key}` with an owner/public access model on `settings.key_value` (two new columns, `owner` and `public`). A super user has full CRUD over every key; a sub user may read its own keys plus any `public` key and may create/update/delete only keys it owns (the owner is always taken from the JWT, never trusted from the request body). Legacy keys created without owner/public — e.g. via the v2 API — have `owner IS NULL` and are treated as owned by the super user and public: readable by everyone, writable only by the super user. Keys remain globally unique (`409` on collision); `GET` lists visible keys or fetches one, `POST` creates (`201`), `PATCH` partially updates value and/or public (`303`), `DELETE` removes (`204`). `GET` also supports a `?paths` projection that returns only selected JSON sub-trees of the value, keyed by path string — comma-separated paths, each a dot-separated segment sequence (e.g. `?paths=user.name,active`); unlike the v2 `paths` parameter, every path segment is bound as a query parameter rather than interpolated into SQL. The v2 `keyvalue`/`configuration` API is unchanged.
...
- New worker-safe (FrankenPHP) v4 OWS endpoint `api/v4/ows/schema/{schema}/database/{database}` — a v4 version of the legacy `/ows`/`/wms` proxy, with full parity (WMS, WFS proxy, UTFGRID/MVT, `filters`, `labels=false`, geofence rules, external WMS-source passthrough). Like the MapCache proxy, the single resource serves every kind of client: a presented Bearer token is validated and must match the database in the path (a bad or foreign token fails instead of being downgraded to anonymous), token-less requests are served anonymously for anonymously-readable layers, and `Read/write` layers challenge with HTTP Basic auth. The legacy endpoint is unchanged. The new path replaces `shell_exec`/`sed` mapfile patching with in-process PHP, drops the php-mapscript dependency (source lookups read `wmssource` from the DB), streams responses without buffering, and cleans up its tmp files. A separate `apiV4.owsRateLimitPerMinute` (default 1200) applies to this path.
- New v4 WFS endpoint `api/v4/wfs/schema/{schema}/database/{database}/srs/[srs]/ts/[timeSlice]`. Like the OWS endpoint, the single resource serves every kind of client: a presented Bearer token is validated and must match the database in the path (a bad or foreign token fails instead of being downgraded to anonymous), while token-less requests serve anonymously-readable layers without credentials; `Read/write` layers (and WFS-T transactions on `Write` layers) are challenged with HTTP Basic auth. Auth is enforced per typeName, the anonymous geofence identity is `*` (so parent-user rules don't match unauthenticated traffic), presented Basic credentials are verified up front (so a fabricated `Authorization` header can't be trusted as identity on GetCapabilities), and the controllers thread an explicit `Connection` end-to-end instead of mutating the process-global database (worker/FrankenPHP-safe).
- Full ("multiple") group-privilege inheritance for layer authorization. A subuser's `usergroup` can name several groups, groups can themselves belong to groups, and the effective privilege is the highest ("best parent") found across the whole inheritance chain (`User::getFullInheritance` walks the group graph breadth-first and is diamond/cycle-safe; `Authorization::extractHighestPrivilege` picks the strongest of `none`/`read`/`read/write`). Ownership is likewise inherited (a user owning any ancestor group named after the schema is treated as owner). Applies to Basic-auth WFS/OWS and the SQL/authorization paths.
- New v4 endpoint to delete a MapCache tileset's cached tiles: `DELETE api/v4/mapcache/database/{database}/tileset/{tileset}` (optionally scoped by `?bbox=minx,miny,maxx,maxy` and `?zoom=minzoom,maxzoom`). It uses a hybrid strategy, because `mapcache_seed -m delete` iterates the whole grid coordinate space (it does not enumerate existing tiles) and so never terminates on a full delete over a deep grid: a **scoped** delete (bbox/zoom bounds the coordinate space) runs `mapcache_seed -m delete` as a detached background job tracked in `settings.seed_jobs` (`202 Accepted` with a job uuid/pid the tileseeder tooling can poll or kill); a **full** delete wipes the backend store directly — `sqlite` empties the tiles in place (`DELETE FROM tiles`) rather than unlinking the file, so a MapCache instance running elsewhere (which caches the open sqlite handle and cannot be reloaded from here) keeps the same file and re-seeds on demand; the delete uses SQLite's truncate optimization so it stays fast even with millions of tiles (`200`). `disk` renames the tileset directory and removes it in a background `rm` (`202`), and `s3`/`memcache` (which can't be wiped cheaply here) return `400` steering to a scoped delete or a TTL/lifecycle rule. Deletion requires write/owner authorization (Bearer token or HTTP Basic; superuser, schema owner, or `read/write` on the layer) regardless of the layer's read-auth level. Replaces the partial legacy `/controllers/setting`-era deletion, which only covered sqlite/disk (bdb was broken, s3/memcache unsupported) and used a blocking, injection-prone `exec("rm -R …")`.
- New authorizing v4 MapCache proxy `api/v4/mapcache/database/{database}/...`, mirroring the OWS proxy so tiles are no longer served straight from MapCache without authentication. It accepts anonymous, HTTP Basic and Bearer-token clients, resolves the requested tileset to its GC2 layer (`schema.table`, vector `.mvt`/`.json` suffix stripped) for WMS/WMTS KVP, WMTS RESTful, TMS and Google Maps, and authorizes read access with the same model as OWS (anonymous only for `None`/`Read`/`Write` layers, HTTP Basic challenge for `Read/write`, per-user/group privilege for tokens). A tile request whose tileset cannot be resolved fails closed (403). Because a tilecache is hit many times per second, the allow decision is cached in Redis for 60s keyed by a hash of the credentials, database and tileset. Authorized requests are streamed from the configured `mapCache.host` backend. Note: the direct Apache `/mapcache/` route is unchanged and must be restricted at the infrastructure level for this authorization to be effective.
- New v4 Map API `api/v4/map/schema/{schema}`, the token-authenticated port of the legacy GUI `/controllers/setting/extent` endpoint. `GET` returns the per-schema initial map view (`center`, `zoom`, `extent`); `PATCH` sets any subset of them (partial update leaves the others intact, `null` clears a value). The API surface uses **EPSG:4326** (`center` as `[lon, lat]`, `extent` as `[minx, miny, maxx, maxy]`); values are transformed at the API boundary and stored in **EPSG:3857** in the same `settings.viewer` document (`center`/`zoom`/`extents` keyed by schema), so existing readers (the legacy GUI endpoint, Mapfile, schema rename) keep working unchanged. The new model methods take the schema explicitly instead of the process-global connection schema (worker/FrankenPHP-safe).

### Changed
- **Scheduler locking rewritten on Postgres advisory locks.** The lock files in `app/tmp` (and the `purge_locks.php` cron) are gone. `get.php` takes a session advisory lock per job (an overlapping tick exits 0 and records a `skipped` run) and one of `gc2scheduler.maxJobs` run slots (default `20`; extra jobs wait), and registers itself in `started_jobs`, which is now the run registry (`status`, `started_at`, `heartbeat`, `finished_at`, `host`, `slot`, `exit_reason`). A run whose process dies loses its locks immediately and is marked `lost` by the reaper; `stale` flags a `running` row without a progress signal for 5 minutes. Runs are spawned under `timeout -s SIGINT -k 60 20h` and finish their registry row on SIGINT/SIGTERM. The lock session verifies at startup that it is session-sticky and refuses to run behind a transaction-pooled PgBouncer (advisory locks would silently be lost). Migration: `started_jobs` gains the new columns and an index, historical rows get their real `started_at`, and `clean_scheduler_tables.php` now also deletes finished runs older than 30 days.
- Scheduler WFS imports page by default: a plain WFS 2.0.0 `GetFeature` URL (no `|` grid notation, no explicit `startIndex`) is fetched in pages of `count` (default 10 000) with `startIndex` and an automatic `sortBy` picked from `DescribeFeatureType` (`id`, `fid`, `gid`, `objectid`, … or a `*_id` property). Jobs using the `|` notation or WFS 1.x behave as before. The `_gc2scheduler` working schema is now created before the job's transaction so ogr2ogr can see it on a database that never ran a job.
- CLI scripts that loop over all databases (`snapshot_worker.php`, `function_worker.php`, `function_event_dispatcher.php`, `function_scheduler.php`, `clean_scheduler_tables.php`) release each database's PDO connection with the new `Model::disconnect()` instead of accumulating one per database.
- `Job::runJob` builds the `get.php` command line with `escapeshellarg` on every value. Before, `url`, `schema`, `type` and `encoding` were interpolated raw; with the new v4 job API those fields are writable over a bearer token, which made a manual start remote code execution.

- Add fallback ordering on meta.fields by `_value` when `_order` is empty.
- The MapCache Apache config (`mapcache.conf`, the per-database `MapCacheAlias` list) is now kept in sync by a PHP cron job (`app/scripts/mapcache_conf.php`, every minute) instead of the `watch_mapcache_changes.sh` busy-loop + `reload.js` Node watcher. The cron script rewrites the config and gracefully reloads Apache both when the set of `{db}.xml` files changes and when the *content* of an existing xml changes (a sha1 fingerprint of the xml files is embedded in `mapcache.conf` as a comment, so an edit that leaves the alias list identical still triggers a reload — mod_mapcache only picks up xml changes on reload). It writes atomically, guards against overlapping runs with a self-releasing `flock` (no stale lock file), and — crucially — validates the result with `apachectl configtest` and reverts if it is invalid, so a broken MapCache xml (e.g. an unsupported cache backend) can no longer take Apache down; it just doesn't get an alias until fixed. Database names are taken from the file basename, so names containing dots work. Trade-off: a newly provisioned database's tiles become reachable up to a minute later instead of near-instantly.
- The v4 OWS endpoints now cache the per-layer allow decision in Redis for 60s (as the MapCache proxy does), keyed by a hash of the identity and the layer, so a cache hit skips the authentication lookup and the per-layer authorization check — OWS/WMS tiles are hit many times per second. Only authenticated allows are cached (Bearer token on `api/v4/ows/schema/{schema}`, HTTP Basic on the no-token endpoint); a wrong password is never cached (it is challenged before the cache is written), and anonymous requests are deliberately not cached so that switching a layer to `Read/write` stops anonymous access immediately rather than after the TTL.
- HTTP Basic authentication for WFS/OWS now checks the primary auth system first. Previously it only matched the separate per-database "viewer" password in `settings.viewer`; it now first verifies the user's login password in the users table (legacy md5 or bcrypt, plus the configured master password), mirroring `Session::start()`, and only then falls back to the viewer password. The fallback is gated by `App::$param['httpBasicViewerFallback']` (default `true`) — set it to `false` to drop the viewer password entirely once it is no longer used. `BasicAuth::authenticate()` and `verifyCredentials()` route through a shared `verifyPassword()` helper so the primary/viewer/flag logic lives in one place.
- The Layer API now also rewrites the per-database MapCache config when a layer changes — but only when the change touches a caching-relevant property (`cache`, `format`, `ttl`, `auto_expire`, `meta_size`, `meta_buffer`, `layers`, `s3_tile_set`). Styling changes (classes, styles, labels) and other layer properties only rewrite the WMS/WFS mapfiles, as before. Generating the MapCache config scans every layer in the database and is expensive, so this gate keeps it from being rewritten on irrelevant changes. The regeneration is best-effort (a failure is logged, not surfaced) and a no-op when the config content is unchanged.
...
- **MapCache config generation refactored and made worker-safe**, following the earlier Mapfile refactor: the 500-line `ob_start()`/inline-PHP template in the `Mapcachefile` controller is replaced by a thin controller delegating to a new `app\models\Mapcachefile` model with unit-tested render helpers. The model takes an injected `Connection` instead of the process-global database, the metadata `<url>` uses the configured host instead of `$_SERVER['HTTP_HOST']`, and generation is split from the checksum-guarded file write. Output is content-identical apart from CDATA/whitespace normalization (schema-tileset titles now use CDATA, and stray template whitespace inside table-tileset titles is gone), so the first run after upgrading rewrites the file once. Also silences PHP 8 warnings from the old template (undefined include-filter variables, `strpos(null)`, property access on empty layer defs).
- **Front-end asset build migrated from Grunt to plain Node scripts.** `Gruntfile.js` and every `grunt-*` dependency are removed, replaced by `build.mjs` (`node build.mjs production` / `dev`) and `hacks.mjs`. The build produces **byte-for-byte identical** output to the old `grunt production` (verified across all 18 generated files: JS bundles, minified CSS, Hogan templates, processed HTML/PHP and cache-busted references), using the same underlying libraries the grunt plugins wrapped — `less`, `clean-css`, `jshint` (dev only), `hogan.js`, `uglify-js`, `htmlprocessor`, `preprocess` — plus a faithful in-repo port of `grunt-cache-bust`. `hacks.mjs` applies the phpfastcache vendor patches (formerly `shell:hacks`); `BUILD_SKIP_SHELL=1` gives an asset-only build with no PHP/OS steps. The Docker build now runs `node build.mjs production && node hacks.mjs` and no longer installs `grunt-cli`.
- **The `/dashboard` PWA build toolchain now runs on Node 24 (LTS)** instead of Node 14. `.nvmrc` and `engines.node` bumped to 24; `--openssl-legacy-provider` is set on the webpack 4 build/start scripts (its hashing uses an OpenSSL algorithm removed in OpenSSL 3 / Node 17+); `cheerio` is pinned via `overrides` so the Jest 23 runtime can load the dependency tree (newer transitive deps use the `node:` protocol Jest 23 cannot resolve); and a `dashboard/.npmrc` (`legacy-peer-deps=true`) lets a plain `npm install` succeed on npm 7+ despite the 2019-era peer-dependency conflicts.

### Fixed
- Scheduler: due jobs did not start from cron on installations where php lives in `/usr/local/bin`. `Job::runJob` spawned `get.php` with a bare `php`, which cron's minimal `PATH` could not resolve (a manual run from a shell worked). The spawn now uses the running interpreter (`PHP_BINARY` under the CLI; under FPM/Apache, where `PHP_BINARY` is php-fpm, the `php` next to it), and `scheduler.php` passes the same binary for `scheduler_run_job.php`.
- OpenAPI for the scheduler API: the `{id}`/`{uuid}` path parameters of `getSchedulerJob`/`getSchedulerRun` are optional (omit to list), `SchedulerJob` carries no required fields (it is the response and the PATCH body) while the new `SchedulerJobInput` schema is the POST body, `postSchedulerRun` requires `job`, and the job `name` property documents how names are normalised (ASCII, lower case, runs of `/ _ | + space -` collapsed to `_`).
- Scheduler `get.php`: a curl failure now ends the run with exit 1 and a `failed` registry row instead of falling through and overwriting the destination table with a partial download; "no data" paths exit 0 after cleanup instead of continuing.
- Scheduler `get.php`: `Content-Type` sniffing (`get_headers`) has a 30 s timeout.
- v4 `checkScope()` answers `403 SUPER_USER_ONLY` instead of a 500 when a sub-user calls a super-user endpoint.
- Apache: `SetEnv ap_trust_cgilike_cl 1` in the vhost so `mod_proxy_fcgi` keeps the backend's `Content-Length` on `HEAD` and `206` responses (needed for ranged Parquet reads; requires an image rebuild).
- CORS: `Range`/`If-Range` are allowed request headers and `Content-Length`, `Content-Range`, `Accept-Ranges`, `ETag`, `Last-Modified`, `Location` are exposed.
- WFS `GetCapabilities` no longer produces invalid XML under FrankenPHP/Caddy. `Util::thePath()` built the operation endpoint base URL from `REDIRECT_URL ?? REQUEST_URI`; under Apache mod_rewrite `REDIRECT_URL` is path-only, but under FrankenPHP only `REQUEST_URI` is set and it carries `?SERVICE=WFS&REQUEST=GetCapabilities`, so the raw `&` landed unescaped in every `xlink:href` and browsers/QGIS rejected the document (`EntityRef: expecting ';'`). `thePath()` now strips the query string so the base URL is clean on both servers.
- The v4 router now matches routes when optional trailing segments are omitted. A route like `api/v4/wfs/schema/{schema}/database/{database}/srs/[srs]/ts/[timeSlice]` previously only matched when the full path was given; now `.../schema/public/database/mydb`, `.../srs/4326` and the full form all match. A literal label (`srs`, `ts`) directly followed by an optional segment counts as optional; required `{...}` segments still 404 when missing. The WFS routes also gained an explicit `ts` label before the `[timeSlice]` segment, and the matcher is extracted into a unit-tested `Route2::matchSignature()`.
...
- **Docker dev image: Node 24 is now on the system PATH for every shell.** nvm only exposes `node`/`npm` in login/interactive shells, so non-login build steps (e.g. building the dashboard via `docker exec` without `-l`) found no node — or fell back to a stale system npm — breaking the build. The Dockerfile now symlinks nvm's `node`/`npm`/`npx` into `/usr/local/bin` after `nvm install 24`, and `ENV NVM_DIR` uses an absolute path (`/root/.nvm`) since Docker does not expand `~`.
- **Removed the unused `grunt-npm-install` dependency**, which pulled `npm@3.10.10` into the repo-root `node_modules`. Because `dashboard/` is nested under the repo root, npm prepends every ancestor `node_modules/.bin` to the PATH of lifecycle scripts, so the dashboard's `preinstall` version check (`npm -v`) resolved to the vendored npm 3.10.10 and failed the build ("You need npm version @>=5 but you have 3.10.10") even on Node 24.
