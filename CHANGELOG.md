...
- `GET /api/v4/schemas` returns `table_count` (tables and views, what `/tables` lists) on every schema, also with `namesOnly=true`, so a client can list schemas with their sizes without loading every table definition; the full listing is unchanged.
