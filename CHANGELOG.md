...
- New no-token v4 OWS/WFS endpoints for anonymous and HTTP-Basic clients (e.g. QGIS): `api/v4/ows/schema/{schema}/database/{database}` and `api/v4/wfs/schema/{schema}/database/{database}/srs/[srs]/ts/[timeSlice]`. Anonymously-readable layers are served without credentials; `Read/write` layers (and WFS-T transactions on `Write` layers) are challenged with HTTP Basic auth against the per-database viewer password. Auth is enforced per layer, the anonymous geofence identity is `*` (so parent-user rules don't match unauthenticated traffic), presented Basic credentials are verified up front (so a fabricated `Authorization` header can't be trusted as identity on GetCapabilities), and the controllers thread an explicit `Connection` end-to-end instead of mutating the process-global database (worker/FrankenPHP-safe). OpenAPI documents both WFS endpoints.
