...
wmsClasses.init = function(record) {
	...
	wmsClasses.proxy = new Ext.data.HttpProxy(
			{
				api : {
					read : '/controller/classes/' + screenName + '/getall/'
							+ wmsClasses.table,
					create : '/controller/classes/' + screenName
							+ '/createcolumn/' + wmsClasses.table,
...
wmsClasses.onAdd = function() {
	var requestCg = {
		url : '/controller/classes/' + screenName + '/insert/'
				+ wmsClasses.table,
		method : 'post',
...
wmsClass.init = function(id) {
	...
	wmsClass.store = new Ext.data.JsonStore( {
		// store config
		autoLoad : true,
		url : '/controller/classes/' + screenName + '/get/' + wmsClasses.table
				+ '/' + id,
		baseParams : {
			xaction : 'read'
		},
	...
	wmsClass.grid = new Ext.grid.PropertyGrid(
			{
				...
				customEditors : {
					...
					'label_size' : new Ext.grid.GridEditor(
							new Ext.form.NumberField( {
								decimalPrecision : 0,
								decimalSeparator : '¤'// Some strange char
							// nobody is using
									}), {}),
					'overlaycolor' : new Ext.grid.GridEditor(
							new Ext.form.ColorField( {}), {}),
					'overlayoutlinecolor' : new Ext.grid.GridEditor(
							new Ext.form.ColorField( {}), {}),
					'overlaysymbol' : new Ext.grid.GridEditor(
					...
					'overlaysize' : new Ext.grid.GridEditor(
							new Ext.form.NumberField( {
								decimalPrecision : 0,
								decimalSeparator : '¤'// Some strange char
							// nobody is
					...
					'overlaywidth' : new Ext.grid.GridEditor(
							new Ext.form.NumberField( {
								decimalPrecision : 0,
								decimalSeparator : '¤'// Some strange char
							// nobody is using
				...
				tbar : [ {
					...
					var requestCg = {
						url : '/controller/classes/' + screenName + '/update/'
								+ wmsClasses.table + '/' + wmsClass.classId,
						params : param,
						headers : {
