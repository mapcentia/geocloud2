...
class _point extends geometryfactory
{
	...
	var $shapeArray;
	var $geomType;
	function __construct($wkt,$srid)// constructor. wkt is set
	{
...
class multilinestring extends geometryfactory
{
	...
	var $shapeArray;
	var $geomType;
	function __construct($wkt)// constructor. wkt is set
	{
		$this->wkt=$wkt;
...
class gmlConverter
{
	...
	*/
	function gmlToWKT($gml,$splitTag=array("FEATUREMEMBER"))
	{
        ...
        $gml = preg_replace('/ xmlns(?:.*?)?=\".*?\"/', "", $gml); // Remove xmlns with "
        $gml = preg_replace('/ xmlns(?:.*?)?=\'.*?\'/', "", $gml); // Remove xmlns with '
        $gml = preg_replace('/ xsi(?:.*?)?=\".*?\"/', "", $gml); // remove xsi:schemaLocation with "
        $gml = preg_replace('/ xsi(?:.*?)?=\'.*?\'/', "", $gml); // remove xsi:schemaLocation with '
        $gml = preg_replace('/ cs(?:.*?)?=\".*?\"/', "", $gml); //
        $gml = preg_replace('/ cs(?:.*?)?=\'.*?\'/', "", $gml);
        $gml = preg_replace('/ ts(?:.*?)?=\".*?\"/', "", $gml);
        $gml = preg_replace('/ decimal(?:.*?)?=\".*?\"/', "", $gml);
        $gml = preg_replace('/\<wfs:(?:.*?)/', "<", $gml);
        $gml = preg_replace('/\<gml:(?:.*?)/', "<", $gml);
        $gml = preg_replace('/\<ogc:(?:.*?)/', "<", $gml);
        $gml = preg_replace('/\<ns:(?:.*?)/', "<", $gml);
