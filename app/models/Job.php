...
class Job extends \app\inc\Model
{
    ...
    public function createCronJobs()
    {
        ...
        foreach ($jobs["data"] as $job) {
            $cmd = "crontab -l | { cat; echo '{$job["min"]} {$job["hour"]} {$job["dayofmonth"]} {$job["month"]} {$job["dayofweek"]} php ". __DIR__ . "/../scripts/get.php {$_SESSION['screen_name']} {$job["schema"]} {$job["name"]} \"{$job["url"]}\" {$job["epsg"]} {$job["type"]} {$job["encoding"]} {$job["id"]} > ".__DIR__."/../../public/logs/{$job["id"]}_scheduler.log'; } | crontab - 2>&1";
            $out = exec($cmd);
            if ($out){
                return $out . " ({$job["id"]})";
            }
