...
class Job extends \app\inc\Model
{
    ...
    {
        $sql = "INSERT INTO jobs (db, name, schema, url, cron, epsg, type, min, hour, dayofmonth, month, dayofweek, encoding, extra, delete_append) VALUES(:db, :name, :schema, :url, :cron, :epsg, :type, :min, :hour, :dayofmonth, :month, :dayofweek, :encoding, :extra, :delete_append, :presql, :postsql)";
        $res = $this->prepare($sql);
        try {
    ...
    public function runJob($id, $db)
    {
        ...
        foreach ($jobs["data"] as $job) {
            if ($id == $job["id"]) {
                ...
                $cmd = " php " . __DIR__ . "/../scripts/get.php {$job["db"]} {$job["schema"]} {$job["name"]} \"{$job["url"]}\" {$job["epsg"]} {$job["type"]} {$job["encoding"]} {$job["id"]} {$job["delete_append"]} " . (base64_encode($job["extra"]) ?: "null") . " " . base64_encode($job["presql"]) . " " . base64_encode($job["postsql"]) . " > " . __DIR__ . "/../../public/logs/{$job["id"]}_scheduler.log\n";
                break;
            }
        }
